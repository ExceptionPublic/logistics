    ...
    layui.use(['form', 'laydate'], function () {
        ...
        form.verify({
            name: function (value, item) { //value：表单的值、item：表单的DOM对象
                var data = ajax("basicJsp/clients/isRepetitionClientsname", {
                    name:value
                });
                console.log(data);
        ...
        //关闭按钮
        form.on('submit(btn_clientsAdd)', function (data) {
            //获取form表单中的字段
            var field = data.field;//form表单字段{name:vlaue}
            // var url="";
            //判断是新增还是修改
            var url = "/basicJsp/clients/clientsAdd";
 ...
 * @param row
 */
function initCliForm(row) {
    console.log(row);
    rowData = row;
    //初始化部门下拉
    // initSelect("personnel/dep/queryDep", [], "#dep", "uuid", "name");
    initForm("clientsAdd", row);
